...
transcribe <path> [lang]# split video → transcribe → write .txt output (lang: en|he)
translate <path> [lang] # translate a transcription .txt file → write .txt output (lang: en|he)
models                  # list available Gemini models
use <model>             # switch active Gemini model
exit
```
