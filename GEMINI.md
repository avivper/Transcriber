...
## Running the App
```bash
./Transcriber  # Global launcher with dependency checking
```
...
clear                     # Clear terminal screen and redisplay banner
...
- **`Constants`**: Centralized application assets (e.g., the `BANNER`).
...
- **Imports**: Use relative imports for internal package dependencies to avoid circularity in package facades.
